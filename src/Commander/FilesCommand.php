...
Options:
  ...
  --help, -h         Show this help message';
    ...
    private function showStatistics(SqliteStorage $storage): void
    {
        ...
        ');
        if (false === $stmt) {
            throw new StorageException('Failed to execute query for file type statistics');
        }
    ...
    private function displayFiles(SqliteStorage $storage, array $files): void
    {
        ...
        foreach ($files as $file) {
            ...
            if (isset($file['is_entry']) && is_bool($file['is_entry']) && $file['is_entry']) {
                $line .= ', [ENTRY]';
            }

            $this->logger->info($line);

            if (isset($file['class_name']) && is_string($file['class_name']) && '' !== $file['class_name']) {
